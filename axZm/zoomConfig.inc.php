...
Since/axZm/axZm.css is updated from time to time as well,
you can override CSS classes in /axZm/axZmCustom.css (add !important if needed) and / or in /axZm/styles/default/style.css; The default style is "default".
...
if(!session_id()){session_start();} unset ($zoom, $zoomTmp); $zoom = array(); $zoomTmp = array();
...
$zoom['config']['galleryOpt'] = array(
	...
	'thumbLiStyle' => array( // Quickly overwrite thumb style (e.g. width and height) without changing css file or write inline styles
		...
		'borderWidth' => 1,
		'marginTop' => $zoom['config']['galleryImgMargin']['top'] - 1,
		'marginBottom' => $zoom['config']['galleryImgMargin']['bottom'] - 1,
		'marginLeft' => $zoom['config']['galleryImgMargin']['left'] - 1
	), 
...
$zoom['config']['spinNoInit'] = array(
	'enabled' => false, // enable / disable
	// message as image file to display e.g. "click_to_load_en.png" or false
...
$zoom['config']['spinSliderHandleSize'] = 15; // int
...
// CSS class name for spin slider
...
$zoom['config']['zoomLoaderClass'] = 'axZm_zoomLoader3';
...
if (isset($_GET['example']) || isset($_GET['previewPic']) || isset($_GET['azImg']) || isset($inludeCustomConfig) || isset($_GET['qq'])){
	...
	if (file_exists(dirname(__FILE__).'/zoomConfigCustom.inc.php')){include_once("zoomConfigCustom.inc.php");}

	// Dynamic settings for the Demo (used only in example1.php)
	if ($zoom['config']['visualConf'] && isset($_GET['example']) && $_GET['example'] == 10){if (file_exists(dirname(__FILE__).'/zoomVisualConf.inc.php')){include_once("zoomVisualConf.inc.php");}}

	// File outside /axZm/ folder
	if (file_exists(dirname(dirname(__FILE__)).'/zoomConfigCustomAZ.inc.php')){include_once(dirname(dirname(__FILE__)).'/zoomConfigCustomAZ.inc.php');}
}
...
// File outside /axZm/ folder
if (file_exists(dirname(dirname(__FILE__)).'/zoomConfigCustomUSR.inc.php')){include_once(dirname(dirname(__FILE__)).'/zoomConfigCustomUSR.inc.php');}
