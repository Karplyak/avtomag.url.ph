...
*/	
	...
	getAllThumbs = function(){
		...
		}else if ($.isFunction(js_beautify)){
			ret = js_beautify(ret, {
				'indent_size': 1,
				'indent_char': '\t'
			});
 		...
 		if (method == 'init'){
			...
			$('#azCropLayer').Jcrop(
				...
				function(){
					...
					jcrop_az.jcropStatus = $('<div id="test" />').css({
						position: 'absolute',
					...
					jcrop_az.jcropFire = $('<div id="test2" />').css({
						// we just take the same button as in "mNavi" toolbar of AJAX-ZOOM
						backgroundImage: 'url('+$('#customBtn_mCustomBtn3').attr('src')+')',
						backgroundSize: '100% 100%',
						position: 'absolute',
	...
	jCropFire = function(){
		...
		if (!jcrop_api){
			...
			if (answer){
				...
				console.log("&x1="+gCV[1][0]+"&y1="+gCV[1][1]+ "&x2="+gCV[1][2]	+ "&y2="+gCV[1][3]);
			}else{
		...
		var tempImg = new Image();
 		$(tempImg).load(imgCropperPathPass, null, function(responseText, textStatus, jqXHR){
...
console.log(">>>><<<<<");
	...
	toggleWYSIWYG = function(num){
		...
		if (!o.data('cleditor')){
			...
			o.parent().remove();
			$('<textarea style="width: 100%; height: 250px">'+v+'</textarea>').appendTo(p);
		}
	},
	...
	setAllEffect = function(num){
		...
		var chkt = $('input[name=effectAll]').attr('checked');
		if (chkt){
	...
	setAllGrav = function(num){
		var t = $('table[data-cropNum='+num+']');
		var chkt = $('input[name=ebGravAll]').attr('checked');
		if (chkt){
	...
	addCropDescrTable = function(img, num){
		...
		<tbody><tr>\
			<td style="vertical-align: top; width: 110px; padding-top: 10px;">\
				<img src="'+img+'" style="max-width: 100px; min-width: 100px; height: auto; border: 2px groove #FFF; cursor: pointer;" onclick="$(\'#cropSlider\').axZmThumbSlider(\'triggerClick\', '+num+'); $(window).scrollTo(0)">\
				<br><input type="button" value="Delete" style="width: 88px; margin-left: 7px; margin-top: 7px;" onclick="$.aZcropEd.deleteCropActionConfirm('+num+')">\
				<br><input type="button" value="WYSIWYG" style="width: 88px; margin-left: 7px; margin-top: 5px;" onclick="$.aZcropEd.toggleWYSIWYG('+num+')">\
				<br><label style="white-space: nowrap; margin-left: 7px; margin-top: 15px; width: 80px;">Language:</label><br>'+makeSelectField($.aZcropEd.langArr, null, 'descrLang', {marginLeft: 7, width: 88}, '$.aZcropEd.changeLang('+num+')').prop('outerHTML')+'\
			</td>\
			<td style="vertical-align: top; padding-top: 10px;">\
				<label style="white-space: nowrap; width: 120px;">Effect:</label>'+makeSelectField(effectObj, selEffect, 'effect', {'marginBottom': 5, 'width': 330}, '$.aZcropEd.setAllEffect('+num+')').prop('outerHTML')+' \
				<input type="checkbox" name="effectAll" onclick="$.aZcropEd.setAllEffect('+num+')"> - for all<br>\
				<label style="white-space: nowrap; width: 120px;">Button position:</label>'+makeSelectField(axZmEbGravity, selebGravity, 'ebGrav',{'marginBottom': 5, 'width': 330}, '$.aZcropEd.setAllGrav('+num+')').prop('outerHTML')+' \
				<input type="checkbox" name="ebGravAll" onclick="$.aZcropEd.setAllGrav('+num+')"> - for all<br>\
				<label style="white-space: nowrap;width: 120px;">Thumb title:</label><input type="text" name="thumb_title" style="margin-bottom: 5px; width: 330px" value="'+thumb_title+'"><br>\
				<label style="white-space: nowrap;width: 120px;">Button title:</label><input type="text" name="button_title" style="margin-bottom: 5px; width: 330px" value="'+button_title+'"><br>\
				<label style="white-space: nowrap">Description (text/html/css and javascript):</label><br>\
				<textarea style="width: 100%; height: 250px" spellcheck="false">'+descr+'</textarea>\
			</td>\
		</tr></tbody></table>';
		
		table = $(table);
		/*
		$.each($.aZcropEd.langArr, function(k, v){
			var lang = v.val;
			if (lang == 'default'){
				table.data('title', button_title);
				table.data('thumbTitle', thumb_title);
				table.data('descr', descr);
			}else{
				table.data('title'+'_'+lang, jcrop_az.allCrops[num].title+'_'+lang || '');
				table.data('thumbTitle'+'_'+lang, jcrop_az.allCrops[num].thumbTitle+'_'+lang || '');
				table.data('descr'+'_'+lang, jcrop_az.allCrops[num].descr+'_'+lang || '');
			}
		});*/
	...
	showRealQueryPath = function(retString){
		$('#aZcR_pathToParameter').addClass('azMsg').
